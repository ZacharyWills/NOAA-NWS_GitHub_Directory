...
do
    echo "## $i"
    echo "<details>"
    echo "<summary>Click to $i Repositories</summary>"
    #this command gets the repos for the orgs and places them into markdown below the org 
    curl -s https://api.github.com/orgs/$i/repos?per_page=200 | grep full_name | awk '{ print "* "$2"\n" }' | sort | tr -d '"' | sed 's/,*$//g'
